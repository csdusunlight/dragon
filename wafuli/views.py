...
#                'task_list': task_list, 
    ...
    task_list = list(Task.objects.filter(type='junior').order_by("-state")[0:2])
    if len(task_list)==2:
        context.update(task1=task_list[0],task2=task_list[1])
    task_list = list(Task.objects.filter(type='middle').order_by("-state")[0:2])
    if len(task_list)==2:
        context.update(task3=task_list[0],task4=task_list[1])
    task_list = list(Task.objects.filter(type='senior').order_by("-state")[0:2])
    if len(task_list)==2:
